...
def objective(x):
    ...
    print(f"return value: {torch.tensor(fs, dtype=torch.float64).shape[-1]}") # return.shape[-1] = 10
    return torch.tensor(fs, dtype=torch.float64)
...
In the course of the programme, you will receive new avatars. We ask you to rate these avatars bit by bit with the help of sliders, which you will see in a moment. \n\n

Whenever your rating is finished, the programme will generate a new avatar adapted to your rating. This process is repeated several times until the final result is presented to you at the end of the programme. \n\n

The generation of the avatars may take some time depending on the performance of the system. We therefore ask for your patience.""", interactive=False, show_label=False)
                if(SCALES == 1):
                    inp1 = gr.Slider(0.0, 1.0, step=0.0001, value=0.11, label="acceptance", info="0 = low | 1 = high", visible=True)
                    inp2 = gr.Slider(0.0, 1.0, step=0.0001, value=0.28, label="likeability", info="0 = low | 1 = high", visible=True)
                    inp3 = gr.Slider(0.0, 1.0, step=0.0001, value=0.28, label="empathy", info="0 = low | 1 = high", visible=True)
                    inp4 = gr.Slider(0.0, 1.0, step=0.0001, value=0.28, label="anthropomorphism", info="0 = low | 1 = high", visible=True)
                    inp5 = gr.Slider(0.0, 1.0, step=0.0001, value=0.28, label="trust", info="0 = low | 1 = high", visible=True)
                if(SCALES == 2):
                    inp1 = gr.Slider(0.0, 1.0, step=0.0001, value=0.11, label="openness", info="0 = low | 1 = high", visible=True)
                    inp2 = gr.Slider(0.0, 1.0, step=0.0001, value=0.28, label="conscientiousness", info="0 = low | 1 = high", visible=True)
                    inp3 = gr.Slider(0.0, 1.0, step=0.0001, value=0.28, label="extraversion", info="0 = low | 1 = high", visible=True)
                    inp4 = gr.Slider(0.0, 1.0, step=0.0001, value=0.28, label="agreeableness", info="0 = low | 1 = high", visible=True)
                    inp5 = gr.Slider(0.0, 1.0, step=0.0001, value=0.28, label="neuroticism", info="0 = low | 1 = high", visible=True)
                if(SCALES == 3):
                    inp1 = gr.Slider(0.0, 1.0, step=0.0001, value=0.11, label="efficiency", info="0 = low | 1 = high", visible=True)
            out = gr.Image()
